...
Result.merge = (a) => (b) =>
  Result.switch((state) => new Failure(b.input, state.error, state.level))(
...
Result.bichain = (f) => (g) =>
  Result.switch((state) => Result.merge(f(state.error))(state))((state) =>
    Result.merge(g(state.value))(state)
  );

Result.chain = (g) =>
  Result.continue((state) => Result.merge(g(state.value))(state));

Result.map = (g) =>
  Result.continue((state) => Result.merge(Success.of(g(state.value)))(state));

Result.mapError = (f) =>
  Result.switch((state) => Result.merge(Failure.of(f(state.error)))(state))(
    identity
  );
...
Failure.of = (error, level) =>
  new Failure(
    null,
    error,
    typeof level === "undefined" ? Failure.LEVEL_INPUT : level
  );

const validate = (validator) => (input) => validator(Result.from(input));

const chain = (...validators) =>
  Result.continue((initialState) =>
    validators.reduce(
      (state, validator) =>
...
const map = (f) => (validator) =>
  Result.continue((state) => Result.map(f)(validator(state)));

const mapError = (f) => (validator) =>
  Result.continue((state) => Result.mapError(f)(validator(state)));

////

const required = Result.chain((value) =>
  value === 0 || (value && (!Array.isArray(value) || value.length > 0))
    ? Success.of(value)
...
const stripWS = Result.chain((value) =>
